...
def variables_setting():
    ...
    global low_white, high_white, low_yellow, high_yellow, low_red, high_red, low_table, high_table, vs
    low_white = np.array([0, 0, 180], dtype=np.uint8)
    high_white = np.array([255, 30, 255], dtype=np.uint8)
...
def initialize_setting():
    ...
    global low_table, high_table, tl, tr, bl, br
    global raw
    global radius_ini, radius2_ini, radius3_ini
    global kkk
    ...
    while current_t <= initialization_time:
        ...
        if len(cnts) > 0:
            ...
            if radius > 10:
                ...
                ball_list_white = np.vstack([b, ball_list_white])
        ...
        if len(cnts2) > 0:
            ...
            if radius2 > 10:
                ...
                ball_list_yellow = np.vstack([b2, ball_list_yellow])
        ...
        if len(cnts3) > 0:
            ...
            if radius3 > 10:
                ...
                ball_list_red = np.vstack([b3, ball_list_red])

        center4 = (int(((bl[0]+tr[0])/2)), int(((bl[1]+tr[1])/2)))
        cv2.circle(frame, center4, 5, (0, 0, 255), -1)


    counter_white = Counter(ball_list_white[:,1])
    ...
    counter_yellow = Counter(ball_list_yellow[:, 1])
    ...
    counter_red = Counter(ball_list_red[:, 1])
...
def recog_balls():
    ...
    #circle_shape = circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 100, param1=90, param2=10, minRadius=15, maxRadius=22)
    ...
    if len(cnts) > 0:
        ...
        if radius > 10 and radius < radius_ini * kkk:
            # draw the circle and centroid on the frame,
            # then update the list of tracked points
            cv2.circle(frame_ROI, (int(x), int(y)), int(radius),
                       (0, 0, 255), 2)
            cv2.circle(frame_ROI, center, 5, (0, 0, 255), -1)
        b = np.array([radius, x, y])
        b_r = np.array([(real_coordinate(radius, 0, tr[0], bl[0], table_real_length) + real_coordinate(radius, 0, tr[1], bl[1], table_real_height))/2,
                        real_coordinate(x, (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length),
                        real_coordinate(y, (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)])
    ...
    if len(cnts2) > 0:
        ...
        if radius2 > 10 and radius2 < radius2_ini * kkk:
            # draw the circle and centroid on the frame,
            # then update the list of tracked points
            cv2.circle(frame_ROI, (int(x2), int(y2)), int(radius2),
                       (0, 0, 255), 2)
            cv2.circle(frame_ROI, center2, 5, (255, 255, 255), -1)
        b2 = np.array([radius2, x2, y2])
        b2_r = np.array([(real_coordinate(radius2, 0, tr[0], bl[0], table_real_length) + real_coordinate(radius2, 0, tr[1], bl[1], table_real_height))/2,
                        real_coordinate(x2, (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length),
                        real_coordinate(y2, (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)])
    ...
    if len(cnts3) > 0:
        ...
        if radius3 > 10 and radius3 < radius3_ini * kkk:
            # draw the circle and centroid on the frame,
            # then update the list of tracked points
            cv2.circle(frame_ROI, (int(x3), int(y3)), int(radius3),
                       (0, 255, 255), 2)
            cv2.circle(frame_ROI, center3, 5, (255, 255, 255), -1)
        b3 = np.array([radius3, x3, y3])
        b3_r = np.array([(real_coordinate(radius3, 0, tr[0], bl[0], table_real_length) + real_coordinate(radius3, 0, tr[1], bl[1], table_real_height))/2,
                        real_coordinate(x3, (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length),
                        real_coordinate(y3, (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)])
    center4 = (int(x_table), int(y_table))
    cv2.circle(frame, center4, 5, (0, 0, 255), -1)
    b4 = np.array([radius_table, x_table, y_table])
    b4_r = np.array([(real_coordinate(radius_table, 0, tr[0], bl[0], table_real_length) + real_coordinate(radius_table, 0, tr[1],
                                                                                                   bl[1],
                                                                                                   table_real_height)) / 2,
                    real_coordinate(x_table, (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length),
                    real_coordinate(y_table, (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)])
...
def billiard_rule():
    ...
    global d_a_r, circle_shape, frame_ROI, bbb, shape_counter_memory




    if game_num == 0:
        result_array = np.hstack([round(game_num), round(a_r[-1, 1]/10), round(a_r[-1, 2]/10), round(a_r[-1, 4]/10), round(a_r[-1, 5]/10), round(a_r[-1, 7]/10), round(a_r[-1, 8]/10), ','])
        game_num = 1
    ...
    if game_phase == 1:
        ...
        if len(a) <= shot_loss_length:
            ...
            shot_param = error_check(a_r[-1 - shot_loss_length, 0], a_r[-1, 0], shot_rate) * error_check(
                a_r[-1 - shot_loss_length, 1], a_r[-1, 1], shot_rate) * error_check(a_r[-1 - shot_loss_length, 2], a_r[-1, 2],
                                                                                shot_rate) * error_check(
                a_r[-1 - shot_loss_length, 3], a_r[-1, 3], shot_rate) * error_check(a_r[-1 - shot_loss_length, 4], a_r[-1, 4],
                                                                                shot_rate) * error_check(
                a_r[-1 - shot_loss_length, 5], a_r[-1, 5], shot_rate) * error_check(a_r[-1 - shot_loss_length, 6], a_r[-1, 6],
                                                                                shot_rate) * error_check(
                a_r[-1 - shot_loss_length, 7], a_r[-1, 7], shot_rate) * error_check(a_r[-1 - shot_loss_length, 8], a_r[-1, 8],
                                                                                shot_rate) * error_check(
                a_r[-1 - shot_loss_length, 9], a_r[-1, 9], shot_rate) * error_check(a_r[-1 - shot_loss_length, 10], a_r[-1, 10],
                                                                                shot_rate) * error_check(
    ...
    # end checker part
        ...
        if len(a) <= end_loss_length:
            ...
            end_param = error_check(a_r[-1 - end_loss_length, 0], a_r[-1, 0], end_rate) * error_check(
                a_r[-1 - end_loss_length, 1], a_r[-1, 1], end_rate) * error_check(a_r[-1 - end_loss_length, 2], a_r[-1, 2],
                                                                              end_rate) * error_check(
                a_r[-1 - end_loss_length, 3], a_r[-1, 3], end_rate) * error_check(a_r[-1 - end_loss_length, 4], a_r[-1, 4],
                                                                              end_rate) * error_check(
                a_r[-1 - end_loss_length, 5], a_r[-1, 5], end_rate) * error_check(a_r[-1 - end_loss_length, 6], a_r[-1, 6],
                                                                              end_rate) * error_check(
                a_r[-1 - end_loss_length, 7], a_r[-1, 7], end_rate) * error_check(a_r[-1 - end_loss_length, 8], a_r[-1, 8],
                                                                              end_rate) * error_check(
                a_r[-1 - end_loss_length, 9], a_r[-1, 9], end_rate) * error_check(a_r[-1 - end_loss_length, 10], a_r[-1, 10],
                                                                              end_rate) * error_check(
        ...
        if switch_end == 0:
            ...
            if end_param == 1:
                if current_t - time_end >= time_end_length:
                    ...
                    white_x = a_r[-1, 1]# real_coordinate(a[-1, 1], (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length)
                    white_y = -a_r[-1, 2] #-real_coordinate(a[-1, 2], (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)

                    yellow_x = a_r[-1, 4] #real_coordinate(a[-1, 4], (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length)
                    yellow_y = -a_r[-1, 5] #-real_coordinate(a[-1, 5], (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)

                    red_x = a_r[-1, 7] #real_coordinate(a[-1, 7], (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length)
                    red_y = -a_r[-1, 8] #-real_coordinate(a[-1, 8], (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)

                    stack = np.hstack(
                        [int(game_num), round(white_x/10, 0), round(white_y/10, 0), round(yellow_x/10, 0), round(yellow_y/10, 0),
                         round(red_x/10, 0), round(red_y/10, 0), ','])
                    print('raw : ', raw)
                    print('stack : ', stack)
                    ...
                    for i in circle_shape[0]:
                        bbb_x = real_coordinate(i[0], (tr[0] - bl[0]) / 2, tr[0], bl[0], table_real_length)
                        bbb_y = -real_coordinate(i[1], (tr[1] - bl[1]) / 2, tr[1], bl[1], table_real_height)
                        bbb_r = real_coordinate(i[2], 0, tr[0], bl[0], table_real_length)
                        if error_check(a_r[-1, 0], bbb_r, 5) == 1 and error_check(a_r[-1, 1], bbb_x, 30) == 1 and error_check(a_r[-1, 2], bbb_y, 30) == 1:
                            shape_counter_memory_white = shape_counter
                            a_r[-1, 0], a_r[-1, 1], a_r[-1, 2] = circle_shape[shape_counter_memory_white, 2], circle_shape[shape_counter_memory_white, 0], circle_shape[shape_counter_memory_white, 1]
                        ...
                        #if error_check(a_r[-1, 6], bbb_r, 5) == 1 and error_check(a_r[-1, 7], bbb_x, 30) == 1 and error_check(a_r[-1, 8], bbb_y, 30) == 1:
                        #    shape_counter_memory_red = shape_counter
                        #    a_r[-1, 6], a_r[-1, 7], a_r[-1, 8] = circle_shape[shape_counter_memory_red, 2], circle_shape[shape_counter_memory_red, 0], circle_shape[shape_counter_memory_red, 1]
                        shape_counter = shape_counter + 1
                        # print(real_coordinate(i[0], (tr[0]-bl[0])/2, tr[0], bl[0], table_real_length), -real_coordinate(i[1], (tr[1]-bl[1])/2, tr[1], bl[1], table_real_height))
            elif end_param == 0:
                time_end = time.time() - t0
def show_video():
    ...
    global key_value
    #frame_ROI = frame[bl[1]:tr[1], bl[0]:tr[0]]
    #cv2.circle(frame_ROI, (int(raw[1]), int(raw[2])), int(radius_ini/2 + radius_ini/2 * math.sin(current_t * 5)), (170, 200, 200), -1)
    #cv2.circle(frame_ROI, (int(raw[3]), int(raw[4])), int(radius2_ini/2 + radius2_ini/2 * math.sin(current_t * 5)), (35, 180, 255), -1)
    #cv2.circle(frame_ROI, (int(raw[5]), int(raw[6])), int(radius3_ini/2 + radius3_ini/2 * math.sin(current_t * 5)), (255, 5, 255), -1)
...
def submain_func():
    ...
    print("radius_ini ", radius_ini)
    print("radius2_ini ", radius2_ini)
    print("radius3_ini ", radius3_ini)


#print(bl, br, tl, tr)
#print('file_name : ', file_1_name)
#print('file_directory : ', file_1_directory)
#print(a)